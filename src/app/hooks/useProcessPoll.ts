...
import { OFManifest, OFApplication, OFApplicationInfo, Application, Process } from "../model/Shapes";
    ...
    const interval = setInterval(async () => {
      ...
      const processMap = applicationsInfo.map(async(info) => {
        ...
        viewEntities.forEach(async (entity) => {
          const parentWindow = await fin.View.wrapSync({uuid: entity.uuid || '', name: entity.name}).getCurrentWindow();
          entity.parent = parentWindow.identity;
        });

        const windows = await Promise.all(windowEntities.filter((window) => (
          !appInfo.isPlatform || (appInfo.uuid !== window.name)
        )).map(async (window) => {
          ...
          const views = viewEntities.filter((view) => (
            view.parent?.uuid === window.uuid && view.parent?.name === window.name
          )).map(async (view) => {
